...
//#include <Mmsystem.h>   // Äîáàâèòü winmm.lib â Project Properties -> Linker -> Command Line -> Additional Options
...
enum direction { LEFT, UP, RIGHT, DOWN }; //óïðîâëåíèå 

enum { END, STOP, PLUS, MOVE };  //íàïðîâëåíèå, äîáàâëåíèå ... 
...
char* GetName(char* st)// èìÿ ïîëüçîâàòåëÿ
{
	SetColor(Blue, Black);
...
void PrintApple(Game &g) // ðàíäîì äëÿ ÿáëîê
{
	int i, x, y;
	...
	{
		x = rand() % 77 + 3;
		y = rand() % 25 + 3; //êîðäèíàòû ÿáëîêà
		for (i = 0; i < n; i++)
		{
			if (x == g.snakes.t[i].X && y == g.snakes.t[i].Y) // ïðîâåðêà ÷òîá ÿáëîêî íå ïîïàëî íà çìåþ
				break;
		}

	}while (i < n);
	g.apples.X = x;
	g.apples.Y = y; //çàïîìèíàåì ïîçèöèþ ÿáëîêà

	SetConsoleCursorPosition(hStdOut, g.apples); //ïåðåíîñèì êóðñîð â ýòó ïîçèöèþ

	SetConsoleTextAttribute(hStdOut, 0x0c); //öâåò ÿáëîêà  
	cout << "Q"; //ðèñóåì ÿáëîêî
}

void StartSnake(Game &g) //ñòàðò çìåéêè è åå êîîðäèíàûò è ñêîðîñòü
{
	system("cls");
	g.snakes.ACount = 3; //ñêîëüêî òî÷åê â çìåéêå
	g.snakes.t = new COORD[3];//ñîçäàëè òî÷êè
	for (int i = 0; i < 3; i++)
	{
		g.snakes.t[i].X = 20 + i;
		g.snakes.t[i].Y = 20;
	...
	PrintApple(g);//ðèñóåì ÿáëîêî
}

void TaskAndWishes()//óñëîâèå
{
...
void SnakeConcol()//ðèñóåì çàñòàâêó
{
	SetColor(Blue, Black);
	ShowCursor(false);
...
void StartGame()//Ðèñóåì êàðòó
{
	...
	GotoXY(84, 28); cout << "Press -> ....(Back to menu)" << endl;
	{
		...
		GotoXY(2, 1); //ðèñóåì âåðõíþþ ãîðèçîíòàëüíóþ ëèíèþ-ñòåíêó
		...
		{
			GotoXY(2, 29); //ðèñóåì íèæíþþ ãîðèçîíòàëüíóþ ëèíèþ-ñòåíêó
			int m = 0;
			for (m = 0; m < 80; m++)
			{
				cout << "_";
	...
	}
	{
		...
		GotoXY(81, 4); cout << "|" << endl;//ðèñóåì ïðàâóþ âåðòèêàëüíóþ ëèíèþ-ñòåíêó
...
{
	int &n = g.snakes.ACount;
	COORD head = g.snakes.t[n - 1]; //ãîëîâà
	COORD tail = g.snakes.t[0]; //õâîñò
	COORD next;
	...
	if (WOLL == true)
	{
		if (next.X < 3 || next.Y < 3 || next.X > 80 || next.Y > 28)//íå óïåðëàñü ëè â ñòåíó
			return STOP;
	}
	...
	else
	{
		if (next.X <= 2)
			...
			next.X = 3;
		else if (next.Y <= 1) // åñëè óïåðëèñü â ñòåíó ïåðåõîä íà äðóãóþ ñòîðîíó
			next.Y = 28;
		else if (next.Y >= 29)
			next.Y = 2;
	}
	...
	{
		for (int i = 0; i < n; i++)
			if (next.X == g.snakes.t[i].X && next.Y == g.snakes.t[i].Y) //íå íàåõàëè ëè íà ñåáÿ
				return END;
	}

	if (next.X == g.apples.X && next.Y == g.apples.Y)
	{
		COORD *tmp = new COORD[++n]; //íîâûé ìàñèâ áîëüøèé íà 1
		for (int i = 0; i < n; i++)
			tmp[i] = g.snakes.t[i]; //ïåðåêîïèðóåì
		tmp[n - 1] = next; //äîáàâëÿåì îäíó
		delete[] g.snakes.t;
		g.snakes.t = tmp;

		SetConsoleCursorPosition(hStdOut, head);

		SetConsoleTextAttribute(hStdOut, 0x0a); //çàêðàøûâàåì ÿáëîêî êîòîðîå ñúåëè
		cout << "O";
	...
	SetConsoleCursorPosition(hStdOut, tail);//çàêðàøèâàåì õâîñò
	cout << " ";

	SetConsoleCursorPosition(hStdOut, head);

	SetConsoleTextAttribute(hStdOut, 0x0a);//ðèñóåì õâîñò çìåè
	cout << "O";

	SetConsoleCursorPosition(hStdOut, next);

	SetConsoleTextAttribute(hStdOut, 0x0a); //ðèñóåì  ãîëîâó
	cout << "@";
	return MOVE;
}

void PrintOptions() // îïöèè èãðû
{
	system("cls");
	ShowCursor(false);
...
void ProduceGame()                                  // ïðîèçâîäèòåëü èãðû
{
	ShowCursor(false);
	do
	{
			...
			case 1:
				...
				while (key != 27)
				{
					while (!_kbhit())
					{
						...
						case PLUS:
							++count;
							g.pause -= 3;   // îâè÷àåò çà ñêîðîñòñü
